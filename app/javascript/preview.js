document.addEventListener('turbo:load', function () {
  // 新規投稿・編集ページのフォームを取得
  const dressForm = document.getElementById('new_dress');
  ...
  const buildNewFileField = () => {
    // 新しいfile_fieldのラッパー要素を生成
    const fileFieldWrapper = document.createElement('div');
    fileFieldWrapper.setAttribute('class', 'file-field-wrapper');

    // 新しいfile_fieldを作成
    const newFileField = document.createElement('input');
    newFileField.setAttribute('type', 'file');
    newFileField.setAttribute('name', 'dress[images][]');
    newFileField.setAttribute('style', 'width: 350px;'); // 幅を設定

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="dress[images][]"]:last-child');
    ...
    // 生成したfile_fieldをラッパーに追加
    fileFieldWrapper.appendChild(newFileField);

    // ラッパーをfileFieldsAreaに追加
    const fileFieldsArea = document.querySelector('.click-upload');
    fileFieldsArea.appendChild(fileFieldWrapper);
  };
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="dress[images][]"]');
